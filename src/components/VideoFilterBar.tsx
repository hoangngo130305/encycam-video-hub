...
const REVIEW_RESULTS = FILTER_OPTIONS.filter(
  (f) => f.group === "review_results",
);
...
export function VideoFilterBar({
  ...
  const isReviewResultSelected = REVIEW_RESULTS.some(
    (f) => f.id === selectedFilter,
  );
  const selectedLabel =
    FILTER_OPTIONS.find((f) => f.id === selectedFilter)?.label || "Tất cả";
      ...
      <div className="flex gap-1.5 flex-wrap items-center">
        ...
        <div ref={dropdownRef} className="relative">
          <button
            onClick={() =>
              setExpandedGroup(
                expandedGroup === "review_results" ? null : "review_results",
              )
            }
            className={cn(
          ...
          >
            ...
            <ChevronDown
              ...
              className={cn(
                ...
                expandedGroup === "review_results" && "rotate-180",
              )}
            />
          </button>
