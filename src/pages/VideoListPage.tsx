...
export default function VideoListPage() {
  const { currentUser, videos, setVideos } = useAppStore();
  ...
  const titleMap: Record<string, string> = {
    ...
    final: 'Tất cả video', admin: 'Tất cả video',
  };
                ...
                <tbody>
                  {filtered.map((v, i) => (
                    <tr key={v.id} onClick={() => navigate(`/videos/${v.id}`)}
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs text-gray-500 dark:text-gray-500">{timeAgo(v.updatedAt)}</span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
