...
export default function VideoListPage() {
  ...
  if (role === "btv")
    visibleVideos = videos.filter((v) => v.btv.id === currentUser.id);
  ...
  const filtered = useMemo(() => {
    return visibleVideos.filter((v) => {
      ...
      const allowedStatuses =
        FILTER_STATUS_MAP[statusFilter] || FILTER_STATUS_MAP.all;
                      ...
                      >
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-10 h-7 rounded-lg bg-gradient-to-br ${v.thumbGradient} flex-shrink-0 flex items-center justify-center`}
                            >
                              <Video size={11} className="text-white/80" />
                            </div>
                            <div>
                              <div className="font-semibold text-sm text-gray-900 dark:text-gray-100 max-w-[280px] truncate">
                                {v.title}
                              </div>
                        ...
                        <td className="px-4 py-3">
                          <span className="font-mono text-xs font-bold text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/40 px-2 py-0.5 rounded">
                            v{v.currentVersion}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        {role !== "btv" && (
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              <Avatar
                                name={v.btv.name}
                                initials={v.btv.initials}
                                bg={v.btv.avatarBg}
                                color={v.btv.avatarColor}
                                size="xs"
                              />
                          ...
                          {v.reviewer ? (
                            <div className="flex items-center gap-1.5">
                              <Avatar
                                ...
                                initials={v.reviewer.initials}
                                bg={v.reviewer.avatarBg}
                                color={v.reviewer.avatarColor}
                                size="xs"
                              />
                              <span className="text-xs text-gray-700 dark:text-gray-300">
                                {v.reviewer.name}
              ...
              {filtered.map((v) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div
                      className={`w-12 h-9 rounded-lg bg-gradient-to-br ${v.thumbGradient} flex-shrink-0 flex items-center justify-center mt-0.5`}
                    >
                      <Video size={13} className="text-white/80" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-sm text-gray-900 dark:text-gray-100 line-clamp-2 leading-snug">
                        {v.title}
                      </div>
