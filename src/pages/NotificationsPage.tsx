...
export default function NotificationsPage() {
  const { currentUser, notifications, markAllRead, markOneRead, unreadCount } = useAppStore();
  ...
  return (
    <div className="flex flex-col h-full">
      <TopBar title="Thông báo" subtitle={unread > 0 ? `${unread} chưa đọc` : 'Tất cả đã đọc'}
        actions={unread > 0 ? (
          <Button variant="ghost" size="sm" icon={<CheckCheck size={13} />} onClick={() => markAllRead(role)}>
              ...
              return (
                <Card key={n.id}
                  ...
                  <div className="flex items-start gap-3">
                    <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0', NOTIF_COLORS[n.type])}>
                      <Icon size={14} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          ...
                          <span className="text-xs text-gray-400 dark:text-gray-600 whitespace-nowrap">{n.createdAt}</span>
                        </div>
                      </div>
